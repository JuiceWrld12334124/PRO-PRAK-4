...
if (isset($_FILES['banner']) && !empty($_FILES['banner'])) {
    $btarget_dir = 'users/'.$data['nickname'].'/'. 'banner/';
    $btarget_file = $btarget_dir.'bannerImage.png';


    if (move_uploaded_file($_FILES['banner']['tmp_name'], '../../'.$btarget_file)) {
        $updateBanner = $pdo->prepare('UPDATE users SET banner_url=:banner WHERE id=:id');
        $updateBanner->bindParam(':banner', $btarget_file);
        $updateBanner->bindParam(':id', $id);
...
if (isset($_FILES['picture']) && !empty($_FILES['picture'])) {
    $ptarget_dir = 'users/'.$data['nickname'].'/'. 'pictures/';


    if (move_uploaded_file($_FILES['banner']['tmp_name'])) {
        $updateBanner = $pdo->prepare('UPDATE users SET r_url=:banner WHERE id=:id');
        $updateBanner->bindParam(':banner', $btarget_file);
        $updateBanner->bindParam(':id', $id);
        $updateBanner->execute();
        echo 'The file '.basename($_FILES['banner']['name']).' has been uploaded.';
